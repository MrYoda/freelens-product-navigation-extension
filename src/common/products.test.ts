...
import { copyPreferences, defaultConfig, expandButtonUrl, normalizeConfig, parseConfig, parseConfigBlock } from "./products.ts";
...
test("copies proxied preferences into a plain draft", () => {
  const proxied = new Proxy(defaultConfig, {});
  const draft = copyPreferences(proxied);
  assert.deepEqual(draft, defaultConfig);
  assert.notEqual(draft, proxied);
});
