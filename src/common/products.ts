...
/** Create a plain snapshot from the JSON-only settings model, including MobX proxies. */
export function copyPreferences(value: ProductNavigationPreferences): ProductNavigationPreferences {
  return JSON.parse(JSON.stringify(value)) as ProductNavigationPreferences;
}
